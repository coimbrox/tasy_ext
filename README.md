...
- quando reconhece o tipo do erro, aponta a **tela nativa certa do Tasy** ou a correção específica em vez de um conselho genérico — certificado/assinatura → Gerenciador de Certificado Digital, cadastro do usuário e Assinatura digital; acesso/permissão negada → Perfis → Funções, Consultas Funções x Usuário / Usuário x Perfil e Restringir acesso; FK, lock ou objeto de banco com problema → Consultas Inconsistências, Lock banco, Jobs e Objetos Inválidos; erro em `GERAR_COLUNAS_SQL_RELAT` (botão "Gerar Campos SQL" do Gerenciador de Relatórios) → consulta de relatório sem limite de caracteres: usar `SUBSTR` e alias curto/sem acento;
...
- **Modo inspeção**: exibe um botão "Inspecionar" fixo na tela; ao clicar em qualquer elemento, abre uma janela com **Contexto da função** (função aberta, painel/view/tabela, parâmetros encontrados no escopo e regras detectadas — cor/visibilidade) e, abaixo, o escopo AngularJS completo. O bloco Contexto da função termina com um **"Onde configurar no Tasy"**: os caminhos prontos (Configurações de Utilização para regras de atributo/legendas/permissões, Parâmetros função, BDM/Handlebar/Tab, e Consultas → Log alteração / Log exclusão para a tabela identificada), já preenchidos com a função e a tabela detectadas.
