...
- quando reconhece o tipo do erro, aponta a **tela nativa certa do Tasy** em vez de um conselho genérico — certificado/assinatura → Gerenciador de Certificado Digital, cadastro do usuário e Assinatura digital; acesso/permissão negada → Perfis → Funções, Consultas Funções x Usuário / Usuário x Perfil e Restringir acesso; FK, lock ou objeto de banco com problema → Consultas Inconsistências, Lock banco, Jobs e Objetos Inválidos;
